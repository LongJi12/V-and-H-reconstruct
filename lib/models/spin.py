...
class Regressor(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, is_train=False, J_regressor=None):
        ...
        for i in range(n_iter):
            ...
            # frame_pred_cam = {'frame_idx': i, 'pred_cam': pred_cam.tolist()}
            # all_pred_cam.append(frame_pred_cam)
        ...
        output = [{
            'theta'  : torch.cat([pred_cam, pose, pred_shape], dim=1),
            #'theta'  : torch.cat([pred_cam], dim=1)
            'verts'  : pred_vertices,
            'kp_2d'  : pred_keypoints_2d,
            'kp_3d'  : pred_joints,
            'rotmat' : pred_rotmat
        ...
        # with open('pred_cam_data.json', 'w') as f:
        #     json.dump(all_pred_cam, f)
        # print('f"Saved pred_cam data to "')
        return output
