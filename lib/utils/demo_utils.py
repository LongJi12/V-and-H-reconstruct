...
#         img_folder = osp.join('./', osp.basename(vid_file).replace('.', '_'))
...
def video_to_images(img_folder, return_info=False):
    # 检查文件夹是否存在
    if not os.path.exists(img_folder):
        raise FileNotFoundError(f"The specified folder does not exist: {img_folder}")

    # 获取图片文件列表，假设文件是以.jpg结尾
    image_files = [f for f in os.listdir(img_folder) if f.endswith('.png')]
    image_files.sort()  # 确保文件是按顺序排列的

    # 读取第一张图片来获取图片尺寸信息
    img_shape = cv2.imread(osp.join(img_folder, image_files[0])).shape if image_files else None

    if return_info:
        # 返回图片文件夹路径、图片数量和图片尺寸
        return img_folder, len(image_files), img_shape
    else:
        return img_folder
...
def images_to_video(img_folder, output_vid_file):
    ...
    command = [
        'ffmpeg', '-framerate', '30000/1001', '-y', '-threads', '16', '-i', f'{img_folder}/%010d.png', '-profile:v', 'baseline',
...
def prepare_rendering_results(results, nframes):
