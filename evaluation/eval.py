...
class Detect3DEval(object):
    ...
    def _prepare(self):
        ...
        mesh_path = opj(base_path,'mesh111')
    ...
    def accumulate(self, p = None):
        ...
        abs_tp_sum = np.cumsum(abs_tps, axis=1).astype(dtype=np.float64)
        abs_fp_sum = np.cumsum(abs_fps, axis=1).astype(dtype=np.float64)
        rel_tp_sum = np.cumsum(rel_tps, axis=1).astype(dtype=np.float64)
        rel_fp_sum = np.cumsum(rel_fps, axis=1).astype(dtype=np.float64)
